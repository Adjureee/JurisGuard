            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {user?.profile_image_path && (
                <button
                  ...
                  disabled={isUploading}
                  className="rounded-lg border border-line bg-card px-3 py-1.5 text-xs font-semibold text-gray-600 transition duration-200 hover:bg-parchment-100 disabled:cursor-not-allowed disabled:opacity-70"
                >
          ...
          {mfaSecret && !user?.mfa_enabled && (
            <div className="mt-4 space-y-3">
              <div>
                ...
                <p className="mt-1 break-all rounded-lg bg-parchment-100 px-3 py-2 font-mono text-sm text-gray-900">
                  {mfaSecret}
                </p>
              </div>
              <div>
                ...
                <p className="mt-1 break-all rounded-lg bg-parchment-100 px-3 py-2 font-mono text-xs text-gray-900">
                  {mfaUri}
                </p>
              </div>
          ...
          {(mfaSecret || user?.mfa_enabled) && (
            <div className="mt-4 flex flex-col gap-3 sm:flex-row">
              <input
                ...
                className="w-full rounded-lg border border-line px-3 py-2 text-sm text-gray-900 outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 sm:max-w-48"
              />
              <button
                type="button"
                onClick={user?.mfa_enabled ? handleDisableMfa : handleEnableMfa}
                disabled={isMfaBusy || mfaCode.length !== 6}
                className="rounded-lg bg-gray-900 px-3 py-2 text-sm font-semibold text-white transition hover:bg-black disabled:cursor-not-allowed disabled:opacity-70"
              >
