...
const COLORS = ["#2E486F", "#C0972F", "#15803D", "#DC2626", "#6E88B4"];
    ...
    <MainLayout>
      ...
      {isLoading ? (
        ...
        <>
                ...
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={weeklyIntake}>
                      ...
                      <YAxis allowDecimals={false} stroke="#6B7280" fontSize={12} />
                      <Tooltip />
                      <Bar dataKey="total_cases" name="Intake" radius={[8, 8, 0, 0]} fill="#2E486F" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              )}
              ...
              {(workload?.status_breakdown ?? []).length === 0 ? <EmptyState message="No personal status breakdown is available yet." /> : (
                <div className="space-y-3">
                  {(workload?.status_breakdown ?? []).map((row) => (
                    <div key={row.status} className="flex items-center justify-between rounded-lg border border-slate-300 bg-card px-3 py-2">
                      <span className="text-sm font-semibold text-gray-900">{row.status}</span>
                      <span className="rounded-full bg-brand-50 px-2.5 py-1 text-xs font-bold text-brand-600">{row.total_cases}</span>
                    </div>
                  ))}
                </div>
              )}
                  ...
                  {caseTypes.slice(0, 6).map((row, index) => (
                    <div key={row.label}>
                      <div className="mb-1 flex justify-between text-sm">
                        ...
                        <span className="text-gray-500">{row.total_cases}</span>
                      </div>
                    ...
                    <button
                      ...
                      className={`flex w-full items-center gap-3 rounded-lg border p-3 text-left transition ${
                        selectedBarangay === barangay.barangay
                          ...
                          : "border-line bg-parchment-100 hover:bg-card"
                      }`}
                    >
                      ...
                      <span className="rounded-full bg-card px-2.5 py-1 text-xs font-bold text-brand-600">
          ...
          <div className="mt-6 rounded-xl border border-slate-300 bg-card p-5 shadow-sm">
            <h2 className="text-base font-bold text-gray-900">Export Center</h2>
