...
function Field({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <div>
      ...
      <p className="mt-1 text-sm font-medium text-gray-800">{value || "Not provided"}</p>
    </div>
  );
}
...
export default function ApplicantDetailsPage() {
  ...
  return (
    <MainLayout>
      ...
      {isLoading || !applicant ? (
        <div className="rounded-[14px] border border-line bg-card p-10 text-center text-sm text-gray-600">
          Loading applicant...
        </div>
      ) : (
        <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
          <div className="space-y-4">
            ...
            <section className="rounded-[14px] border border-line bg-card p-5 shadow-sm ">
              <h2 className="mb-4 border-b border-line pb-4 text-base font-semibold text-gray-800">
                Profile Info
              </h2>
              <div className="grid gap-4 md:grid-cols-2">
          ...
          <aside className="h-fit rounded-[14px] border border-line bg-card p-5 shadow-sm ">
            <div className="mb-5 border-b border-line pb-5">
              <h2 className="text-base font-semibold text-gray-800">Employee ID</h2>
              <div className="mt-3 overflow-hidden rounded-xl border border-line bg-parchment-100">
                {employeeIdEvidence.imageSrc ? (
                  ...
                  <div className="flex h-52 flex-col items-center justify-center gap-2 px-4 text-center text-sm text-gray-600">
                  ...
                  <div className="flex h-52 items-center justify-center px-4 text-center text-sm text-gray-600">
              ...
              <button
                ...
                className="mt-3 w-full rounded-lg border border-brand-600 bg-card px-4 py-2 text-sm font-semibold text-brand-600 transition hover:bg-brand-50 hover:text-brand-700 disabled:cursor-not-allowed disabled:opacity-50"
              >
                Preview ID
              </button>
            </div>
            ...
            {isFinalStatus(applicant.approval_status) ? (
              <p className="mt-3 rounded-lg border border-line bg-parchment-100 px-3 py-2 text-sm text-gray-600">
              ...
              <div className="mt-4 grid gap-2">
                ...
                <button
                  ...
                  disabled={isUpdating}
                  className="rounded-lg border border-red-600 bg-card px-4 py-2.5 text-sm font-semibold text-red-600 transition hover:bg-red-600 hover:text-white disabled:opacity-60"
                >
                  Reject
                </button>
              </div>
