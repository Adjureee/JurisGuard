...
function statusClass(status: string) {
  ...
  if (normalizedStatus === "Approved") return "bg-emerald-50 text-emerald-800";
  ...
  return "bg-parchment-100 text-gray-600";
}
    ...
    <MainLayout>
      ...
      <section className="overflow-hidden rounded-xl border border-line bg-card shadow-card">
        <div className="grid gap-3 border-b border-line bg-card px-5 py-4 md:grid-cols-[1fr_220px_auto] md:items-end">
          ...
          <div className="rounded-lg border border-line bg-parchment-100 px-4 py-2 text-sm font-semibold text-gray-600">
      ...
      {reportModalOpen && (
        <ModalPortal>
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/55 px-4 py-6 backdrop-blur-sm">
            <div className="flex max-h-[88vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-card shadow-[0_24px_80px_rgba(15,23,42,0.28)]">
              <div className="flex items-start justify-between gap-4 border-b border-slate-200 bg-card px-5 py-4">
                ...
                <button
                  ...
                  className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-card text-lg font-semibold leading-none text-slate-500 transition hover:bg-parchment-100 hover:text-slate-900"
                  aria-label="Close report dialog"
                >
                  x
                </button>
              </div>
              <div className="min-h-0 flex-1 overflow-y-auto bg-parchment-100/60 px-5 py-5">
                {reportModalMode === "revise" &&
                ...
                <div className="rounded-xl border border-slate-200 bg-card p-4 shadow-sm">
                  <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_170px_170px]">
                    <label>
                      ...
                      <input
                        ...
                        className="mt-1.5 h-10 w-full rounded-lg border border-slate-300 bg-card px-3 text-sm text-slate-950 outline-none transition focus:border-brand-600 focus:ring-4 focus:ring-brand-600/10"
                      />
                    </label>
                    <label>
                      <span className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                  ...
                  <textarea
                    ...
                    className="mt-4 min-h-20 w-full resize-y rounded-lg border border-slate-300 bg-card px-3 py-2 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-brand-600 focus:ring-4 focus:ring-brand-600/10"
                  />
                </div>

                <div className="mt-4 rounded-xl border border-slate-200 bg-card shadow-sm">
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-3">
                    ...
                    <button
                      ...
                      onClick={runPreview}
                      disabled={loading}
                      className="rounded-lg border border-slate-300 bg-card px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-parchment-100 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60"
                    >
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-card px-5 py-4">
          ...
          >
            <div className="jurisguard-modal-surface flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-line bg-card shadow-xl">
              ...
              <div className="min-h-0 flex-1 overflow-y-auto p-6">
                {selected.notes && (
                  <div className="mb-5 rounded-xl border border-line bg-parchment-100 p-4 text-sm text-gray-600">
                    {selected.notes}
                  </div>
                )}
                <VersionSelector
                    ...
                    <div className="mt-3 space-y-3">
                      {selected.feedback.length === 0 ? (
                        ...
                        selected.feedback.map((item) => (
                          <div
                            key={item.feedback_id}
                            className="rounded-lg bg-parchment-100 p-3 text-sm"
                          >
                    ...
                    {isAdmin &&
                      ...
                      selectedStatus !== "Draft" && (
                        <textarea
                          ...
                          className="mt-4 min-h-24 w-full rounded-lg border border-parchment-300 px-3 py-2 text-sm focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20"
                        />
                      )}
                    {isAdmin && !selectedIsLatest && (
                      <p className="mt-4 rounded-lg bg-parchment-100 p-3 text-sm text-gray-500">
              ...
              <div className="flex flex-wrap justify-end gap-2 border-t border-line bg-parchment-100 px-6 py-4">
                <button
                  type="button"
                  onClick={() => setSelected(null)}
                  className="rounded-lg border border-parchment-300 bg-card px-4 py-2 text-sm font-semibold text-gray-800 hover:bg-parchment-200"
                >
                  Close
                </button>
                {!isAdmin && selectedIsLatest && selectedStatus === "Draft" && (
                  <button
                    ...
                    className="rounded-lg border border-brand-600 bg-card px-4 py-2 text-sm font-semibold text-brand-600 hover:bg-brand-50"
                  >
                    Edit Draft
                  </button>
                )}
                ...
                {isAdmin &&
                  ...
                  selectedStatus !== "Draft" && (
                    <button
                      ...
                      className="rounded-lg border border-amber-500 bg-card px-4 py-2 text-sm font-semibold text-amber-800 hover:bg-amber-50"
                    >
                      Request Correction
                    </button>
                  )}
...
function StaffSubmissionTable({
  ...
  return (
    <table className="w-full min-w-[860px] text-sm">
      <thead className="bg-parchment-100 text-xs uppercase tracking-wide text-gray-500">
        <tr>
      ...
      <tbody className="divide-y divide-line">
        ...
        >
          {submissions.map((submission) => {
            const status = normalizeStatus(submission.status);
            return (
              <tr key={submission.submission_id} className="hover:bg-parchment-100">
                ...
                <td className="px-5 py-4">
                  <div className="flex justify-end gap-2">
                    <button
                      ...
                      className="rounded-lg border border-parchment-300 bg-card px-3 py-1.5 text-xs font-semibold text-gray-800 hover:bg-parchment-200"
                    >
                      View
                    </button>
                    {status === "Draft" && (
                      <button
                        ...
                        className="rounded-lg border border-brand-600 bg-card px-3 py-1.5 text-xs font-semibold text-brand-600 hover:bg-brand-50"
                      >
                        Edit
                      </button>
                    )}
              ...
              <button
                ...
                className={`rounded-lg border px-3 py-1.5 text-sm font-semibold transition ${
                  active
                    ...
                    : "border-parchment-300 bg-card text-gray-600 hover:bg-parchment-100"
                }`}
              >
                V{version.version}
              </button>
...
function VersionDetailPanel({
  ...
  return (
    <section className="mb-5 rounded-xl border border-line bg-parchment-100 p-4">
        ...
        <tbody className="divide-y divide-line bg-card">
          {versions.map((version) => {
            ...
            return (
              <tr
                key={version.submission_id}
                className={`cursor-pointer hover:bg-parchment-100 ${active ? "bg-brand-50" : ""}`}
                onClick={() => onOpen(version)}
              >
...
function StaffIdentity({ submission }: { submission: CaseSubmission }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-brand-50 text-xs font-bold text-brand-600">
        {src ? (
      ...
      <div>
        <p className="font-semibold text-gray-800">{submission.staff_name}</p>
        <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-500">
          {submission.staff_role}
        </p>
      </div>
...
function ReportHeader({ submission }: { submission: CaseSubmission }) {
  const src = avatarSrc(submission.staff_profile_image_path);
  return (
    <div className="border-b border-line bg-parchment-100 px-6 py-5">
      <div className="flex items-center gap-4">
        <div className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-full bg-brand-50 text-sm font-bold text-brand-600">
          {src ? (
            <img src={src} alt="" className="h-full w-full object-cover" />
          ) : (
            initials(submission.staff_name)
      ...
      <table className="w-full min-w-[760px] text-sm">
        ...
        <tbody className="divide-y divide-line bg-card">
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <tr key={item.record.case_id} className="hover:bg-parchment-100">
                <td className="px-4 py-3 font-semibold text-gray-800">
                  {item.record.intake_record.control_no}
                </td>
                <td className="px-4 py-3 text-gray-600">{item.client_name}</td>
