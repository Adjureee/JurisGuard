...
function hydrateFilipino(doc: Document, data: PrintableFormData) {
  ...
  setInput(inputs, 6, record.intake_record.region);
  setInput(inputs, 7, record.intake_record.district_office);
  setInput(inputs, 8, record.intake_record.form_date);
  setInput(inputs, 9, record.intake_record.control_no);
  setInput(inputs, 12, caseDetails.assigned_pao);
  setInput(inputs, 13, record.intake_record.nature_of_request);
  setInput(inputs, 14, client.client.name);
  setInput(inputs, 15, client.client.age);
  setInput(inputs, 16, client.client.sex);
  setInput(inputs, 17, client.client.civil_status);
  setInput(inputs, 18, client.client.religion);
  setInput(inputs, 19, client.client.educational_attainment);
  setInput(inputs, 20, client.client.citizenship);
  setInput(inputs, 21, client.client.language_dialect);
  setInput(inputs, 22, details.address);
  setInput(inputs, 23, details.email);
  setInput(inputs, 24, details.contact_no);
  setInput(inputs, 25, details.individual_monthly_income);
  setInput(inputs, 26, details.spouse);
  setInput(inputs, 27, details.address_of_spouse);
  setInput(inputs, 28, details.contact_no_of_spouse);
  setInput(inputs, 29, details.detained_since);
  setInput(inputs, 30, details.place_of_detention);
  setInput(inputs, 31, details.representative_name);
  setInput(inputs, 32, details.representative_age);
  setInput(inputs, 33, details.representative_sex);
  setInput(inputs, 34, details.representative_civil_status);
  setInput(inputs, 35, details.representative_address);
  setInput(inputs, 36, details.representative_contact_no);
  setInput(inputs, 37, details.representative_relationship);
  setInput(inputs, 38, details.representative_email);
  setInput(inputs, 39, classification.flag_pwd ? "PWD" : "");
  setInput(inputs, 40, classification.flag_foreign_national ? "Yes" : "");
  setInput(inputs, 49, client.client.name);
  setInput(inputs, 50, client.client.civil_status);
  setInput(inputs, 51, details.spouse);
  setInput(inputs, 52, details.address);
  setInput(inputs, 53, details.individual_monthly_income);
  setInput(inputs, 63, record.intake_record.applicant_role_other);
  setInput(inputs, 64, record.adverse_party.name);
  setInput(inputs, 65, record.adverse_party.address);
  setInput(inputs, 66, `${caseDetails.title_of_case} ${caseDetails.case_no}`.trim());
  setInput(inputs, 67, caseDetails.court_body);
  ...
  checkNearText(doc, "Administrative", record.intake_record.nature_of_case.toLowerCase().includes("admin"));
  checkNearText(doc, "Senior Citizen", classification.flag_senior);
  checkNearText(doc, "CICL", classification.flag_cicl);
  checkNearText(doc, "Woman", classification.flag_female);
  checkNearText(doc, "VAWC", classification.flag_vawc_victim);
  checkNearText(doc, "Urban Poor", classification.flag_urban);
  checkNearText(doc, "Rural Poor", classification.flag_rural);
