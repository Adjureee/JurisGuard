          ...
          <div className="jurisguard-modal-surface flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-[#E5E7EB] bg-white shadow-xl">
            <div className="flex items-start justify-between gap-4 border-b border-[#E5E7EB] bg-[#F8FAFC] px-6 py-5">
              <div>
                ...
                <p className="mt-1 text-sm text-[#6B7280]">Select coverage, regenerate the snapshot preview, then save or resubmit.</p>
              </div>
              <button type="button" onClick={() => setReportModalOpen(false)} className="rounded-lg border border-[#D1D5DB] bg-white px-3 py-1.5 text-sm font-semibold text-[#2B3642] hover:bg-[#F3F4F6]">Close</button>
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto p-6">
              {reportModalMode === "revise" && editingSubmission?.feedback.length ? (
                <div className="mb-4 rounded-xl border border-[#FEF3C7] bg-[#FFFBEB] p-4">
                  <p className="text-sm font-bold text-[#92400E]">
                    Admin Correction Notes - Version {editingSubmission.version}
                  </p>
                  <div className="mt-2 space-y-2">
                    {editingSubmission.feedback.map((item) => (
                      <p key={item.feedback_id} className="text-sm text-[#4B5563]">{item.comments}</p>
                    ))}
                  </div>
                </div>
              ) : null}
              <div className="grid gap-4 lg:grid-cols-[1fr_180px_180px]">
                <label>
                  <span className="text-xs font-semibold uppercase tracking-wide text-[#6B7280]">Submission Title</span>
                  <input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="May 2026 Intake Report" className="mt-1 h-10 w-full rounded-lg border border-[#D1D5DB] px-3 text-sm focus:border-[#704389] focus:ring-2 focus:ring-[#704389]/20" />
                </label>
                <label>
                  <span className="text-xs font-semibold uppercase tracking-wide text-[#6B7280]">Start Date</span>
                  <input type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)} className="mt-1 h-10 w-full rounded-lg border border-[#D1D5DB] px-3 text-sm focus:border-[#704389] focus:ring-2 focus:ring-[#704389]/20" />
                </label>
                <label>
                  <span className="text-xs font-semibold uppercase tracking-wide text-[#6B7280]">End Date</span>
                  <input type="date" value={dateTo} onChange={(event) => setDateTo(event.target.value)} className="mt-1 h-10 w-full rounded-lg border border-[#D1D5DB] px-3 text-sm focus:border-[#704389] focus:ring-2 focus:ring-[#704389]/20" />
                </label>
              </div>
              <textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Summary notes" className="mt-4 min-h-24 w-full rounded-lg border border-[#D1D5DB] px-3 py-2 text-sm focus:border-[#704389] focus:ring-2 focus:ring-[#704389]/20" />
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                <p className="text-sm text-[#4B5563]">{preview.length} case snapshot(s) in preview.</p>
                <button type="button" onClick={runPreview} disabled={loading} className="rounded-lg border border-[#D1D5DB] bg-white px-4 py-2 text-sm font-semibold text-[#2B3642] hover:bg-[#F8FAFC]">Regenerate Preview</button>
              </div>
              <SnapshotTable items={preview} />
            </div>
            <div className="flex justify-end gap-2 border-t border-[#E5E7EB] bg-[#F8FAFC] px-6 py-4">
              <button type="button" onClick={saveReportModal} disabled={loading || !title} className="rounded-lg bg-[#704389] px-4 py-2 text-sm font-semibold text-white hover:bg-[#5F3675] disabled:opacity-50">
...
function SnapshotTable({ items }: { items: SubmissionSnapshot[] }) {
  return (
    <div className="mt-5 overflow-x-auto rounded-xl border border-[#E5E7EB]">
      <table className="w-full min-w-[760px] text-sm">
        ...
        <tbody className="divide-y divide-[#E5E7EB] bg-white">
          {items.length === 0 ? (
            <tr><td colSpan={5} className="px-4 py-8 text-center text-[#6B7280]">No cases in this report snapshot.</td></tr>
          ) : items.map((item) => (
